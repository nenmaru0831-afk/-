window.Battle = (() => {
  ...
  function spawnGrunts() {
    const phone = isPhone();
    const rows = phone ? (wave === 1 ? 2 : 3) : (wave === 1 ? 4 : 5);
    const cols = phone ? (wave === 1 ? 5 : 6) : (wave === 1 ? 8 : 9);
    const gapX = w / (cols + 1);
    const list = [];
  ...
  function fireDan(x, y, ang, spd, size, spin) {
    const cap = isPhone() ? 42 : 96;
    if (bombs.length > cap) return;
    const slow = spd * (isPhone() ? 0.42 : 0.58);
    let vx = Math.cos(ang) * slow;
    let vy = Math.sin(ang) * slow;
    if (vy < 0.7) vy = Math.max(vy, 0) + 0.85;
    bombs.push({
      x, y,
      vx, v: vy,
      kind: kindNow,
      pumpkin: kindNow === "samhain",
      r: Math.max(7.4, (size || 11) * 0.7),
      spin: spin || 0
    });
  }
  ...
  function tick() {
    ...
    bunkers.forEach((k) => {
      ...
      if (kindNow === "samhain") {
        ...
        ctx.fillStyle = "#a05878";
        ctx.beginPath(); ctx.ellipse(k.x, k.y + 3, bw / 2, bh / 2, 0, 0, Math.PI * 2); ctx.fill();
        ctx.fillStyle = "#f0a8c8";
        for (let i = 0; i < 5; i += 1) {
          const a = (Math.PI * 2 * i) / 5;
          ctx.beginPath();
          ctx.ellipse(k.x + Math.cos(a) * 10, k.y + Math.sin(a) * 6, bw * 0.18, bh * 0.22, a, 0, Math.PI * 2);
          ctx.fill();
        }
      } else if (kindNow === "eishi") {
