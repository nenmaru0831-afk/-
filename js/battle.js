window.Battle = (() => {
  ...
  imgSamhainBoss.src = "assets/images/chars/samhain_boss.webp";
  const imgAmaseBack = new Image();
  imgAmaseBack.src = "assets/images/chars/amase_back.webp";
  const imgKanraBack = new Image();
  imgKanraBack.src = "assets/images/chars/kanra_back.webp";
  const imgBossMap = {
  ...
  imgBossMap.shigure.src = "assets/images/chars/shigure_boss.webp";
  imgBossMap.yozakura.src = "assets/images/chars/yozakura_boss.webp";
  imgBossMap.eishi.src = "assets/images/chars/eishi_boss.webp";
  imgBossMap.tokka.src = "assets/images/chars/tokka_boss.webp";
  let hp = 100;
  let maxHp = 100;
  let shotsFired = 0;
  let shotsHit = 0;
  let totalScore = 0;
  let battleSeedScore = 0;
  let combo = 0;
  let comboT = 0;
  let floats = [];
  let takenHit = false;
  ...
  function spawnGrunts() {
    const phone = isPhone();
    const rows = phone ? (wave === 1 ? 2 : 2) : (wave === 1 ? 4 : 5);
    const cols = phone ? (wave === 1 ? 4 : 5) : (wave === 1 ? 8 : 9);
    const gapX = w / (cols + 1);
    const list = [];
  ...
  function isPhone() {
    return !!(window.Settings && window.Settings.get("device") === "phone");
  }
  function fireDan(x, y, ang, spd, size, spin) {
    const cap = isPhone() ? 22 : 54;
    if (bombs.length > cap) return;
    const slow = spd * (isPhone() ? 0.34 : 0.52);
    let vx = Math.cos(ang) * slow * 0.85;
    let vy = Math.sin(ang) * slow;
    if (vy < 1.15) vy = 1.15 + Math.abs(vy) * 0.25;
    const shapes = kindNow === "samhain" ? ["circle", "diamond"]
      : kindNow === "shigure" ? ["rect", "diamond"]
      : kindNow === "yozakura" ? ["petal", "diamond"]
      : kindNow === "eishi" ? ["tri", "rect"]
      : ["rect", "tri"];
    bombs.push({
      x, y: y + 8,
      vx, v: vy,
  ...
  function fireBossNormal(boss) {
    ...
    if (kindNow === "samhain") {
      ...
      if (phase === 0) {
        for (let p = 0; p < 6; p += 1) {
          for (let i = 0; i < 8; i += 1) fireDan(cx, cy, rot + p * 1.047 + i * 0.05, 2.2 + i * 0.2, 13, 0.012);
        }
      } else if (phase === 1) {
        for (let i = 0; i < 24; i += 1) fireDan(cx, cy, i * 0.262 + rot * 0.5, 2.6, 12, 0);
        for (let i = 0; i < 10; i += 1) fireDan(20 + i * ((w - 40) / 9), 12, Math.PI / 2, 2.8, 12, 0.01);
      } else if (phase === 2) {
        for (let i = -9; i <= 9; i += 1) fireDan(cx, cy, ang0 + i * 0.08, 3.1, 13, 0.008);
      } else {
        for (let i = 0; i < 18; i += 1) fireDan(cx, cy, rot + i * 0.35, 2.4, 12, 0.02);
        for (let i = 0; i < 8; i += 1) fireDan(cx + (i - 3.5) * 16, cy, Math.PI / 2, 3.0, 12, 0);
      }
  ...
  function fireBossArt(boss) {
    ...
    rings.push({ x: boss.x, y: boss.y, r: 8, max: 140, life: 22 });
    ...
    if (id === "dark") {
      ...
      for (let i = 0; i < 28; i += 1) fireDan(10 + Math.random() * (w - 20), 8, Math.PI / 2 + (Math.random() - 0.5) * 0.4, 2.4 + Math.random(), 13, 0.02);
      for (let i = 0; i < 8; i += 1) beam(20 + i * ((w - 40) / 7), 8, 20 + i * ((w - 40) / 7), h - 8, 18, "yozakura");
  ...
  function drawGrunt(f) {
    ...
    if (kindNow === "samhain") {
      ...
      ctx.fillRect(x - s * 0.38, y - s * 0.12, s * 0.76, s * 0.5);
      ...
      ctx.fillRect(x - s * 0.38, y - s * 0.16, s * 0.76, 3);
      ctx.fillStyle = "#fff0e8";
      ctx.fillRect(x - 2, y - s * 0.4, 4, 8);
    } else if (kindNow === "yozakura") {
      ctx.fillStyle = "#c07098";
      ctx.beginPath(); ctx.arc(x, y + s * 0.08, s * 0.22, 0, Math.PI * 2); ctx.fill();
      ctx.fillStyle = "#f0a8c8";
      for (let i = 0; i < 6; i += 1) {
        const a = (Math.PI * 2 * i) / 6 - Math.PI / 2 + tickN * 0.02;
        ctx.beginPath();
        ctx.ellipse(x + Math.cos(a) * s * 0.34, y + Math.sin(a) * s * 0.28, s * 0.2, s * 0.12, a, 0, Math.PI * 2);
        ctx.fill();
      }
      ...
      ctx.beginPath(); ctx.ellipse(x, y + 2, s * 0.46, s * 0.3, 0, 0, Math.PI * 2); ctx.fill();
      ctx.fillStyle = "#1a1028";
      ctx.fillRect(x - s * 0.2, y - s * 0.55, s * 0.4, s * 0.5);
      ctx.fillStyle = "#6b4ad4";
      ctx.fillRect(x - s * 0.14, y - s * 0.32, 3.2, 3);
      ctx.fillRect(x + s * 0.06, y - s * 0.32, 3.2, 3);
      ctx.strokeStyle = "rgba(120,80,200,0.45)";
      ctx.beginPath(); ctx.arc(x, y, s * 0.55, 0, Math.PI * 2); ctx.stroke();
    } else {
      ctx.fillStyle = "#5a121c";
      ctx.fillRect(x - s * 0.4, y - s * 0.08, s * 0.8, s * 0.42);
      ctx.fillStyle = "#8b1e2d";
      ctx.beginPath(); ctx.moveTo(x, y - s * 0.62); ctx.lineTo(x + s * 0.28, y - s * 0.08); ctx.lineTo(x - s * 0.28, y - s * 0.08); ctx.closePath(); ctx.fill();
      ctx.fillStyle = "#e8c484";
      ctx.fillRect(x - 3, y - s * 0.5, 6, 10);
      ctx.fillStyle = "#7a1c28";
      ctx.fillRect(x - 1, y - s * 0.48, 2, 8);
    }
  }
  ...
  function showCutin(who, side, mirror) {
    ...
    if (!box || !img) return;
    const src = {
      amase: "assets/images/chars/amase.webp",
      ...
      tokka: "assets/images/chars/tokka.webp"
    }[who];
    if (!src) return;
    img.src = src;
  ...
  function gameOver(reason) {
    ...
    if (window.Achieve) window.Achieve.unlock("over");
    bankScore();
    if (window.Records) {
  ...
  function tick() {
    ...
    if (kindNow === "tokka" && !joinDone && !joining && !(window.Settings && window.Settings.get("heart"))) {
      ...
      if ((hp <= 30) || (wave >= 3 && left <= 30000 && boss && boss.hp >= 200)) {
        hp = Math.max(hp, 1);
        beginJoinTalk();
        return;
      }
    ...
    bombs.forEach((b) => {
      ...
      if (!b.ally && invuln <= 0 && dx < hitR && dy < hitR) {
        b.y = h + 20;
    ...
    if (wave >= 3 && !bossLive && !winning) {
      ...
      if (window.Achieve) window.Achieve.unlock(kindNow);
      stopBgm();
      hud();
    ...
    if (allyOn && kindNow === "tokka") {
      if (allyCd > 0) allyCd -= 1;
        ...
        const bolt = (x, y, vx, vy, dmg) => shots.push({ x, y, vx, v: vy, dmg: dmg || 7, ally: true, kind: "eishi" });
        if (kind === "aim" && boss) {
          for (let i = -2; i <= 2; i += 1) bolt(ax + i * 10, h - 10, i * 0.4, -7.4, 9);
        } else if (kind === "rain") {
          for (let i = 0; i < 12; i += 1) bolt(20 + i * ((w - 40) / 11), h - 8, 0, -6.8, 6);
        } else if (kind === "ring" && boss) {
          for (let a = 0; a < Math.PI * 2; a += 0.4) bolt(tx, boss.y + 20, Math.cos(a) * 0.2, -5.2, 6);
        } else if (kind === "sweep") {
          for (let i = 0; i < 10; i += 1) bolt(ax, h - 8, -2.6 + i * 0.58, -6.6, 7);
        } else {
          for (let i = 0; i < 6; i += 1) {
            bolt(30, h - 8, 1.2 + i * 0.2, -6.8, 7);
            bolt(w - 30, h - 8, -1.2 - i * 0.2, -6.8, 7);
          }
        }
        if (window.Achieve) window.Achieve.unlock("join");
      }
    }
    ...
    bunkers.forEach((k) => {
      ...
      if (kindNow === "samhain") {
        ctx.fillStyle = "#c45810";
        ctx.beginPath(); ctx.ellipse(k.x, k.y + 2, bw / 2, bh / 2, 0, 0, Math.PI * 2); ctx.fill();
        ctx.fillStyle = "#e07012";
        ctx.beginPath(); ctx.ellipse(k.x, k.y, bw / 2.15, bh / 2.2, 0, 0, Math.PI * 2); ctx.fill();
        ctx.fillStyle = "#1a0900";
        ctx.beginPath(); ctx.arc(k.x - 10, k.y - 4, 3.2, 0, Math.PI * 2); ctx.fill();
        ctx.beginPath(); ctx.arc(k.x + 10, k.y - 4, 3.2, 0, Math.PI * 2); ctx.fill();
        ctx.beginPath(); ctx.moveTo(k.x - 8, k.y + 6); ctx.lineTo(k.x, k.y + 11); ctx.lineTo(k.x + 8, k.y + 6); ctx.stroke();
        ctx.fillStyle = "#2f6b24";
        ctx.fillRect(k.x - 4, k.y - bh / 2 - 8, 8, 10);
      } else if (kindNow === "shigure") {
        ctx.fillStyle = "#4a0c12";
        ctx.fillRect(k.x - bw / 2 - 2, k.y - bh / 2 + 4, bw + 4, bh);
        ctx.fillStyle = "#8a1c24";
        ctx.fillRect(k.x - bw / 2, k.y - bh / 2, bw, bh);
        ctx.fillStyle = "#e8c484";
        ctx.fillRect(k.x - bw / 2, k.y - bh / 2, bw, 5);
        ctx.fillRect(k.x - bw / 2, k.y + bh / 2 - 5, bw, 5);
        ctx.fillStyle = "#fff0c8";
        ctx.fillRect(k.x - 2, k.y - bh / 2 + 6, 4, bh - 12);
      } else if (kindNow === "yozakura") {
        ctx.fillStyle = "#4a2038";
        ctx.fillRect(k.x - bw / 2, k.y - bh / 2, bw, bh);
        ctx.fillStyle = "#f4d0e0";
        ctx.fillRect(k.x - bw / 2 + 4, k.y - bh / 2 + 4, bw - 8, bh - 8);
        ctx.strokeStyle = "#c07098";
        ctx.strokeRect(k.x - bw / 2 + 7, k.y - bh / 2 + 7, bw - 14, bh - 14);
        ctx.fillStyle = "#8a3058";
        ctx.fillRect(k.x - 2, k.y - bh / 2, 4, bh);
      } else if (kindNow === "eishi") {
        ...
        ctx.strokeRect(k.x - bw / 2 + 4, k.y - bh / 2 + 4, bw - 8, bh - 8);
        ctx.fillStyle = "#6b4ad4";
        ctx.fillRect(k.x - 8, k.y - 3, 16, 6);
      } else {
        ctx.fillStyle = "#5a3a18";
        ctx.fillRect(k.x - bw / 2 - 2, k.y - bh / 2 + 3, bw + 4, bh);
        ctx.fillStyle = "#e8c484";
        ctx.fillRect(k.x - bw / 2, k.y - bh / 2, bw, bh);
        ctx.fillStyle = "#7a1c28";
        ctx.fillRect(k.x - 4, k.y - bh / 2, 8, bh);
        ctx.fillStyle = "#fff4d0";
        ctx.fillRect(k.x - bw / 2 + 6, k.y - 2, bw - 12, 3);
      }
      ctx.globalAlpha = 1;
    });
    pickups.forEach((p) => {
    ...
    bombs.forEach((b) => {
      ...
      const sh = b.shape;
      if (sh === "rect") {
        ctx.fillStyle = k === "tokka" ? "#8b1e2d" : k === "eishi" ? "#6b4ad4" : "#c43a3a";
        ctx.fillRect(b.x - r, b.y - r * 0.7, r * 2, r * 1.4);
        return;
      }
      if (sh === "tri") {
        ctx.fillStyle = k === "eishi" ? "#a080ff" : "#e8c484";
        ctx.beginPath();
      ...
      if (sh === "diamond") {
        ctx.fillStyle = k === "yozakura" ? "#f0a8c8" : "#ffb24a";
        ctx.beginPath();
        ctx.moveTo(b.x, b.y - r);
  ...
  function onKey(e) {
    ...
    if (e.type === "keydown" && "456789".includes(e.key) && window.Bag) {
      const rows = window.Bag.listOwned();
      const idx = "456789".indexOf(e.key);
      if (rows[idx]) window.Bag.use(rows[idx].id);
    }
  ...
  function paintJoinLine() {
    ...
    if (!line) return;
    if (name) name.textContent = line[0] === "永視" ? "？？？" : "天瀬 若葉";
    if (text) text.textContent = line[1];
  }

  function endJoinTalk() {
    if (joinAnimLock) return;
    joinAnimLock = true;
    joining = false;
  ...
  function advanceJoin() {
    if (!joining || joinAnimLock) return;
    joinI += 1;
  ...
  function start(kind, win) {
    ...
    const retrying = (over || winning) && kindNow === (kind || "samhain");
    kindNow = kind || "samhain";
    onWin = win;
    if (retrying) totalScore = battleSeedScore;
    else battleSeedScore = totalScore;
    ...
    ["touchMove", "touchShot", "touchSkills"].forEach((id) => {
  ...
  function bindOver() {
    ...
    const pads = [document.getElementById("touchMove"), document.getElementById("touchShot"), document.getElementById("touchSkills")];
    pads.forEach((pad) => {
  ...
  function stop() {
    ...
    const bag = document.getElementById("bagBar");
    if (bag) bag.hidden = true;
    const panel = document.getElementById("battleOver");
    if (panel) panel.hidden = true;
  }

  function paintBattleBag() {
    const bar = document.getElementById("bagBar");
    if (!bar || !window.Bag) return;
    const rows = window.Bag.listOwned();
    bar.hidden = !rows.length || !running;
    bar.innerHTML = rows.map((it) =>
      '<button type="button" data-bag="' + it.id + '">' + it.mark + " " + it.name + " ×" + it.n + "</button>"
    ).join("");
    bar.querySelectorAll("[data-bag]").forEach((btn) => {
      btn.onclick = (e) => {
        e.stopPropagation();
        if (window.Bag) window.Bag.use(btn.dataset.bag);
      };
    });
  }
  window.paintBattleBag = paintBattleBag;

  function useItem(id) {
    const it = (window.Bag && window.Bag.ITEMS || []).find((x) => x.id === id) || { fx: id };
    const fx = it.fx || id;
    if (window.Achieve) window.Achieve.unlock("item");
    if (Math.random() < 0.2) {
      floats.push({ x: player.x, y: player.y - 36, v: -1, life: 40, t: "効果が出なかった！" });
      if (window.Achieve) window.Achieve.unlock("fail");
      paintBattleBag();
      return;
    }
    if (fx.startsWith("heal")) giveHeal(Number(fx.slice(4)) || 10, player.x, player.y - 20);
    else if (fx === "guard") addStatus("guard", "防御", 360, "guard");
    else if (fx === "ward") addStatus("ward", "加護", 600, "ward");
    else if (fx === "haste") addStatus("haste", "加速", 480, "haste");
    else if (fx === "double") addStatus("double", "二連", 480, "double");
    else if (fx === "bomb") {
      bombs = bombs.filter((b) => Math.hypot(b.x - player.x, b.y - player.y) > 80);
      rings.push({ x: player.x, y: player.y - 18, r: 10, max: 80, life: 16 });
    } else if (fx === "purge") {
      bombs = [];
      rings.push({ x: w / 2, y: h / 2, r: 12, max: 160, life: 20 });
    } else if (fx === "time") {
      if (deadline) deadline += 20000;
    } else if (fx === "lamp") { veilT = 0; addStatus("lamp", "灯", 300, "lamp"); }
    else if (fx === "tiny") addStatus("tiny", "針", 360, "tiny");
    else if (fx === "rage") addStatus("rage", "辛", 360, "rage");
    else if (fx === "luck") addStatus("luck", "葉", 480, "luck");
    else if (fx === "regen") addStatus("regen", "再生", 420, "regen");
    else if (fx === "slowb") addStatus("slowb", "鈍弾", 420, "slowb");
    else if (fx === "invuln") { invuln = 150; addStatus("ward", "金鈴", 150, "ward"); }
    else if (fx === "shot") shots.push({ x: player.x, y: player.y - 12, v: -7, vx: 0, dmg: 2 });
    else if (fx === "tea") statuses = statuses.filter((s) => s.id !== "limp");
    else if (fx === "artslow") lastBossAt = Date.now();
    else if (fx === "invert") addStatus("invert", "誤図", 240, "invert");
    else if (fx === "stun") foes.forEach((f) => { if (!f.boss) f.hp = Math.max(0, f.hp - 1); });
    else if (fx === "pact") { hp = Math.max(1, hp - 12); bombs = []; }
    else if (fx === "reflect") addStatus("reflect", "鏡", 300, "reflect");
    hud();
    paintBattleBag();
  }
