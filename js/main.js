(() => {
  ...
  function openDialog(action, mode) {
    ...
    if (action === "start" && window.Story) {
      ...
      if (window.Achieve) window.Achieve.unlock("start");
      window.Story.start(0);
      return;
    }
    if (action === "achieve") {
      if (window.Achieve) window.Achieve.open();
      return;
    }
    if (action === "start" && window.Achieve) window.Achieve.unlock("start");
    if (action === "guideline" && window.Achieve) window.Achieve.unlock("guide");
  ...
  function boardDayStamp() {
    const now = new Date();
    const jst = new Date(now.getTime() + (9 * 60 * 60 * 1000 - now.getTimezoneOffset() * 60000) * 0);
    const h = (now.getUTCHours() + 9) % 24;
    const utc = new Date(now.getTime() + 9 * 3600 * 1000);
    const y = utc.getUTCFullYear();
    const m = utc.getUTCMonth();
    const d = utc.getUTCDate();
    const day = h < 6 ? new Date(Date.UTC(y, m, d) - 86400000) : new Date(Date.UTC(y, m, d));
    const use = h < 6 ? new Date(utc.getTime() - 86400000) : utc;
    return use.getUTCFullYear() + "-" + String(use.getUTCMonth() + 1).padStart(2, "0") + "-" + String(use.getUTCDate()).padStart(2, "0");
  }
  function resetBoardIfDawn() {
    const stamp = boardDayStamp();
    try {
      if (localStorage.getItem("mukai-board-day") !== stamp) {
        localStorage.setItem("mukai-board", "[]");
        localStorage.setItem("mukai-board-day", stamp);
      }
    } catch (e) { /* ignore */ }
  }
  ...
  applyMaint();
  resetBoardIfDawn();
  window.setInterval(() => { applyMaint(); resetBoardIfDawn(); }, 30000);
  ...
  function applyDevice(dev) {
    ...
    if (window.Achieve) window.Achieve.unlock(dev === "phone" ? "phone" : "pc");
  ...
  if (ama && bub) {
    ama.addEventListener("click", (e) => {
      e.stopPropagation();
      bub.hidden = false;
      if (window.Achieve) window.Achieve.unlock("talk");
  ...
  let moonN = 0, verN = 0, typed = "";
  const moon = document.querySelector(".moon");
  if (moon) moon.addEventListener("click", () => {
    moonN += 1;
    if (moonN >= 7 && window.Achieve) window.Achieve.unlock("secret1");
  });
  const ver = document.querySelector(".ver");
  if (ver) ver.addEventListener("click", () => {
    verN += 1;
    if (verN >= 5 && window.Achieve) window.Achieve.unlock("secret2");
  });
  window.addEventListener("keydown", (e) => {
    if (e.key.length === 1) typed = (typed + e.key).slice(-8);
    if (/NEN|nen|夢界/.test(typed) && window.Achieve) window.Achieve.unlock("secret3");
  });
  document.querySelector(".sns-news")?.addEventListener("click", () => { if (window.Achieve) window.Achieve.unlock("news"); });
  document.querySelector(".sns-comment")?.addEventListener("click", () => { if (window.Achieve) window.Achieve.unlock("board"); });
